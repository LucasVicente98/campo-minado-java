...
public class Campo {
	...
	private boolean aberto; // por padrão, recebe o valor falso
	...
	public boolean abrir() { // chamada recurssiva
		if (!aberto && !marcado) {
			if (minado) {
	...
	void reiniciar() {
		...
		minado = false;
		marcado = false;
		notificarObservadores(CampoEvento.REINICIAR); // Cai no default, então será aplicado o estilo padrão (campos
														// fechados)
	}
}
